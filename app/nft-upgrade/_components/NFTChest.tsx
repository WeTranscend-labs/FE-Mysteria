...
interface NFTChestProps {
  onSelect: (nft: TransformedNFT) => void;
  selectedNFT: TransformedNFT | null;
}
...
const mockNFTs: NFT[] = [
  ...
  },
  {
    ...
    quality: 'Rare',
    image:
      'https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/6b78761b-b731-43f6-be47-63d69cd7d2a9/dh2l3xk-65a5c142-8660-4562-9e05-70779ae12324.jpg',
  },
  {
    id: 3,
...
export default function NFTChest({ onSelect, selectedNFT }: NFTChestProps) {
  ...
  const { nfts, isLoading, error, getNFTsMetadata } = useGetNFTs(address);

  const [nftData, setNftData] = useState<TransformedNFT[]>([]);

  useEffect(() => {
    const fetchNFTs = async () => {
      if (nfts) {
        const metadata = await getNFTsMetadata(nfts);
        setNftData(metadata);
      }
    };

    fetchNFTs();
  }, [nfts, getNFTsMetadata]);

  const handleSelect = (nft: TransformedNFT) => {
    onSelect(nft);
    setIsOpen(false);
  };
        ...
        {isOpen && (
          <div className="fixed inset-0 isolate z-90">
            ...
            <div className="fixed inset-0 flex items-center justify-center">
              <motion.div className="relative w-full max-w-4xl mx-4 bg-black">
                {/* Close Button */}
                <button
